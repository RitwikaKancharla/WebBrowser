...
class UrlComparator<Url> implements Comparator<Url>{
...
public class AvlImplementation {	
	void Insert ( Node parent  , int key , AvlTree avl  )
	{
		if ( key < parent.key )	{
	...
	Status findElement ( Node n , int key  , boolean root )
	{
		...
		if ( key < n.key ){ // If going further down the tree is possible
			if ( n.lChild != null )
	...
	int predecessor ( Node n , AvlTree avl ){
